...
**Scenario**:
The situational context configured for a Session — e.g. the caller's reason for calling or goal. Independent of the simulated counterpart's character traits (see Persona). Carries a **Category**: what kind of call it is, one of `operations`, `requirements`, `pricing` or `closing` (docs/adr/0072), four values refining F-03's three call contexts. Display and filter only, never reaches the prompt. Optional: a Scenario may have none.
_Avoid_: Persona, situation. For the Category: type, Szenariotyp (the free-text `scenario_type` column it replaces is gone; see docs/adr/0072)
...
**Turn**:
One exchange within a Session's simulated conversation — the user's spoken utterance and the Persona's spoken reply to it, treated as a single unit rather than as one Turn per speaker. A Session consists of a sequence of Turns, ordered by a sequence index starting at 1. A Turn may lack one half: the opening Turn has no user utterance, because the Persona speaks first. Stored, a Turn is flattened into one row per speaker, so the `turn` table holds utterances; count the user's rows to know how often they spoke.
...
**Measurement**:
One speech metric measured over a Session — over the whole call, or over one marked stretch of it (`call`, `pressure`, `rest`; docs/adr/0081) — e.g. speaking rate or intonation. Never per Turn (docs/adr/0051). Always describes the user's speech, never the Persona's: the Persona's speech is synthesized, so measuring it says nothing about how the user spoke. This is what lets a Session hold both speakers without the attribution becoming ambiguous.
_Avoid_: Metric, score, KPI

**Finding**:
A noteworthy event at one moment of a Session — today, one hard interruption by the user. The counterpart to a Measurement, and like it always about the user's side; an event that happened, never a value judged against a threshold (docs/adr/0051). Its offset is counted from the start of the Session, and is empty where the Finding describes the whole call.
