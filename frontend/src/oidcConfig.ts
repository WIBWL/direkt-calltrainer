...
// vite.config's `envDir`). The names are not VITE_-prefixed: the issuer is the
// same value the backend checks `iss` against, and two copies of it in .env
// under two names is a pair that can disagree, so vite.config widens
// `envPrefix` to let the SPA read the backend's own names.
//
...
export const oidcClientId: string = import.meta.env.OIDC_CLIENT_ID ?? "calltrainer-frontend";
