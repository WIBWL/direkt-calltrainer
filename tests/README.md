...
| Follow-up Scenario drafted on request from a Session's feedback (what the model is and is not given, the route's refusals and idempotency, that it is stored once as the User's own private Scenario, and that it leaves the library when its Session does) | F-60, F-10, ADR 0011, ADR 0031, ADR 0043, ADR 0050, ADR 0051, ADR 0058, ADR 0059, ADR 0066, ADR 0067, ADR 0069 | `test_followup_scenario.py` |
