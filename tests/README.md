...
| Storage consent: the gate on the write path, version staleness, withdrawal deletes | F-49, F-31, ADR 0031, ADR 0034, ADR 0060 | `test_consent.py` |
| Six-month retention: the boundary, the per-account suspension, idempotence | F-49, ADR 0031, ADR 0060, ADR 0061 | `test_retention.py` |
| Data rights: overview counts, export completeness and scoping, deleting one training | F-49, F-31, ADR 0050, ADR 0058, ADR 0060 | `test_data_rights.py` |
| Spoken content stays out of the log unless explicitly switched on | F-49, ADR 0039, ADR 0060 | `test_transcript_logging.py` |
| Deep links into the client-side router survive a reload, without swallowing unknown API paths | F-31, ADR 0009, ADR 0058 | `test_spa_routing.py` |
...
| Session history: ownership as the query, total order under pagination, what the listing withholds | F-13, F-48, F-31, F-50, ADR 0009, ADR 0031, ADR 0051, ADR 0052, ADR 0057, ADR 0058 | `test_session_history.py` |
