...
This keeps the first migration minimal and avoids guessing at access patterns before the Session/Feedback read paths that will actually query this data exist. Foreign-key columns without an explicit index — Postgres does not automatically index the referencing side of a foreign key, unlike the referenced primary-key side — will fall back to sequential scans until a real workload identifies which lookups need one. This decision is expected to be revisited once the read side (Session history, aggregated Feedback per ADR 0024, F-11/F-13) is built against real query patterns rather than guesses.
