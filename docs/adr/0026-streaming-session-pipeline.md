# ADR 0026: Streaming Session Pipeline via Chunked TTS over WebSocket
...
ADR 0013 chose a turn-based, full-request/response pipeline for the MVP — record a full user turn, wait for the complete STT → dialogue → TTS chain, then play back the response — and explicitly deferred real streaming to a later phase, to keep the first implementation simple and to measure the gateway's real latency first. That latency has since proven too slow for the desired conversational feel: waiting for an entire reply to be generated and synthesized before any audio plays back makes each Turn feel like a pause-and-resume exchange rather than a phone call.
