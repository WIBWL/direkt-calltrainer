...
ADR 0048 means those quantities now genuinely exist as rows before any prompt is built. The question is what the model is asked to do with them.

## Decision

We will split the wrap-up into a measurement half and an interpretation half, and give the model only the second. Every number originates from Praat (ADR 0047) or the transcript, is computed before the model is called, and is passed into the prompt as a stated fact. The model is instructed to interpret those findings and explicitly not to estimate, recompute, or introduce quantities of its own.
