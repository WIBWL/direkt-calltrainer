...
Accepted. Narrowed by ADR 0048: Session audio is never persisted, so what lives in this database is transcripts and numbers, not recordings.
...
Two details of the Decision have since moved. Session audio is no longer among what is stored: ADR 0048 measures each Turn inline and releases the waveform when the Turn ends, so the growth in database size and backup volume anticipated above is the growth of transcripts and measurements, not of recordings. And there is no `DATABASE_URL` setting — `build_database_url()` in `backend/db/session.py` assembles the URL from the same `POSTGRES_*` values on first use, and Alembic calls it too, so migrations cannot run against a different database than the app. The mechanism the Decision describes is unchanged; only the ready-made variable it names was dropped, so the password lives in one place instead of being duplicated across `.env` and `compose.yaml`.
