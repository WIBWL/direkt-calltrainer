# ADR 0036: Deterministic Constraint Names and Database-Enforced Vocabularies
...
Introducing a convention retroactively is not free, and the migration that does it is more intricate than it looks: because Alembic applies the convention when *replaying* older revisions, a database created after this change gets different constraint names from one migrated before it. The rename therefore identifies each constraint by its structure — table, kind, column — rather than by its current name, and restores the handful of names that earlier revisions drop explicitly, so the chain still runs in both directions. That subtlety is a one-time cost of adopting the convention late; a project that sets it up front never meets it.
