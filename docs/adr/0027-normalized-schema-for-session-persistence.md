# ADR 0027: Normalized Relational Schema for Session Persistence
...
ADR 0010 and ADR 0026 established Postgres plus SQLAlchemy for Session persistence, but not the actual shape of that data: what a Session, Turn, Persona, Szenario, or Feedback look like as durable rows. The domain glossary (`CONTEXT.md`) already defines Session, Persona, Scenario, and Feedback as first-class concepts, and ADR 0024 anticipates reading Feedback back per-Session and aggregated across Sessions (F-11, F-13) once persistence exists.

## Decision

We modeled persistence as a normalized relational schema of twelve entities, mirroring the domain glossary as closely as possible: `Persona` — together with `PersonaEinwand`, which holds a Persona's typical objections as ordered rows rather than as a repeating group inside the Persona itself — plus `Szenario` and `Sprache` as reusable lookup/configuration entities; `Session` as the central fact table referencing them; `Turn` as one row per utterance within a Session; `MetrikTyp`/`Messung`/`Befund` for per-turn speech analysis, each keyed against a metric-type lookup table; `Feedback`/`Feedbackpunkt` for the post-call summary and its individual points, referencing back to the originating Turns/Befunde; and `AnalysisJob` to track async processing status per Session (see ADR 0033). Table and column names are German, matching the rest of the project's German-language domain vocabulary and documentation conventions. `backend/db/models.py` is the single source of truth: both the Alembic migrations (ADR 0028) and the ER diagram (ADR 0031) are derived from it, not maintained independently. Ownership between entities is expressed as ORM-level delete cascades (`cascade="all, delete-orphan"`) on the owning relationships — Persona over its Einwände, Session over its Turns, Feedback and AnalysisJobs, Turn over its Messungen and Befunde, Feedback over its Feedbackpunkte — so that deleting a Session removes its whole subtree while leaving the shared reference entities (Persona, Szenario, Sprache, MetrikTyp) untouched. The foreign keys themselves carry no `ON DELETE` clause yet, so this holds for deletes going through the ORM, not for raw SQL.
