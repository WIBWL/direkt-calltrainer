...
**The wording is versioned.** `CURRENT_VERSION` changes whenever the notice changes in substance, and every earlier *yes* becomes stale and is asked again. Consent to a text nobody put in front of the person is not consent, and a silent edit must not keep an old "yes" alive.

A **withdrawal is not asked again**, whatever version it was made under. That is the deliberate exception and it was only written in the code until a review pointed out that this paragraph claimed otherwise. `allows_storage` treats a stale withdrawal exactly as it treats a stale grant — nothing is stored either way, which is the answer that matters here — while `decision_required` stays false, so the dialog does not reopen. Re-prompting somebody who has said no, every time the text is edited, turns the dialog into a way of wearing them down; the notice is still on the profile screen for anyone who wants to read it, and granting from there is a decision taken in front of the current text.

**The check happens where the Session is written, not where it starts.** A call runs for minutes; an answer taken at the handshake would be that old by the time the row is written, and someone who withdrew while talking would find the call stored anyway. `backend/session/persistence.py::persist_session` asks inside its own transaction, which is the last point at which unconsented data can still be prevented from existing.

It asked from outside that transaction until a review found the gap: the answer was true, and the INSERT it authorised committed some milliseconds later — long enough for a withdrawal in another tab to record itself and delete every Session that existed *at that moment*. The one being written arrived after the delete and survived it, a stored training under a withdrawn consent that no deletion path would ever visit again. Since then the check reads within the writing transaction, and both sides first take a per-subject advisory lock (`consent.lock_subject`, transaction-scoped, keyed on the `sub`), so a withdrawal and a Session write for the same subject cannot interleave at all. The guard therefore belongs to the writer rather than to its caller, which is also why `persist_session` answers `None` instead of an id when it refuses.
