...
## Amendment (2026-09-06): regenerate on a repeated opening, too

A live call after two barge-ins in a row: the history held two short cut-off lines ([ADR 0035](0035-barge-in-interruption.md)'s trimmed fragments), the user offered a refund, and the persona answered with one of those lines word for word — the oscillation check above then did the only thing it could and ended the call. A fragment is far easier for a 4B model to reproduce exactly than a whole reply, so interruptions make this ending markedly more likely, and it lands on exactly the trainee who is doing well enough to interrupt.

Front 3 is therefore widened by one case: a reply whose **first chunk opens with a sentence the persona has already said verbatim** in this call (first sentence against first sentence, case-insensitive, cut-off dashes ignored, under the same short-line floor as the oscillation check) is caught before any audio is synthesized and regenerated once with its own nudge (`REPEAT_OPENING_NUDGE`, quoting the repeated sentence). A regeneration that loops again is spoken and handled by the post-hoc checks exactly as before — one fresh attempt, not a retry loop. Everything in the Decision above is unchanged; this only moves the *first* detection of the cheapest-to-detect loop to before the speaker.

The same call showed the failure the Decision names — a varied opening sentence with the same block carried underneath — surviving every guard: the opening guard sees only the first sentence, and the restatement check compares with the previous reply only, while the block came from two Turns back. That block is now handled at the chunk, before synthesis: every sentence of a chunk that the persona has already said verbatim in this call (`repetition.drop_said_sentences`, same length floor as the oscillation check, cut-off dashes ignored) is dropped, the new sentences are spoken, and the call goes on. Like the opening checks this runs on a guarded attempt only — a requested repeat, a goodbye and the one regeneration are spoken as generated and judged by the post-hoc checks. So the trainee hears what is new, and a reply that *is* nothing but old blocks reaches the backstop as before, on its regeneration.

Two smaller findings from the same call. The cut-off dash ADR 0035 leaves in the history is copied back out by the model along with the line ("Ich will—"), so every chunk is scrubbed of it before synthesis. And the ADR 0035 nudge for the Turn after an interruption is placed *before* the user's message and quotes nothing — the model continues from whatever text sits last in its context, and a version that ended by quoting the fragment had it finish that sentence over the user's offer.
