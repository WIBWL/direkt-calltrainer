...
Accepted (scope narrowed to dialogue generation by ADR 0022 — see below)
...
In practice, EFRE-Direkt turned out to provide API access for the dialogue/reasoning model only. STT and TTS are not available through it and had to move to separately self-hosted models — see ADR 0022, which narrows this ADR's scope.
